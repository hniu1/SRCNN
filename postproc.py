...
def read_data(var,exp):
    #X_test = np.load(f'./scripts/X_test_fsrcnn_daily_{exp}.npy')
    y_test = np.load(f'./scripts/y_test_fsrcnn_daily_{exp}.npy')
    y_test_predict = np.load(f'./scripts/y_test_predict_fsrcnn_daily_{exp}.npy')
    valloss = np.load(f'./scripts/val_loss_fsrcnn_daily_{exp}.npy')
    trainloss = np.load(f'./scripts/train_loss_fsrcnn_daily_{exp}.npy')
    #return X_test,y_test,y_test_predict,valloss,trainloss
    return y_test,y_test_predict,valloss,trainloss
def plot_map(var,y_test,y_test_predict,exp):
   ...
   # ax[0].set_title("Low-Res (ERA5)")
    ...
    ax[0].remove()
    plt.savefig(f'spatialmaps_{exp}_{var}.pdf')
...
def plot_diff(var,y_test,y_test_predict,exp):
    ...
    plt.savefig(f'spatialmaps_{exp}_{var}_diff.pdf')
...
def plot_loss(var,val,train,exp):
    ...
    plt.title(exp)
    plt.legend()
    plt.savefig(f'{var}_val_train_loss_{exp}.pdf')
